    ...
    var body: some View {
        ...
        .overlay(alignment: .top) {
            if showSuccessMessage {
                Text(isEditing ? "Task edited successfully" : "Task added successfully" )
                    .padding()
                    .background(isEditing ? .blue.opacity(0.9) : .green.opacity(0.9))
...
struct Ejercicio5ViewSheet: View {
    ...
    var isTempTaskNameValid: Bool {
        if let _tempTask = Binding($tempTask) {
            return !_tempTask.name.wrappedValue.isEmpty
        }
        return false
    }
    ...
    var buttonOpacity: Double {
        isUserEditingATask  && !isTempTaskNameValid || !isUserEditingATask && !isNewTaskTitleValid ? 0.5 : 1
    }
            ...
            VStack(spacing: 20) {
                ...
                TextField("Name of the task", text: $title)
                    ...
                    .onAppear {
                        ...
                        title = _tempTask.wrappedValue.name
                    }
                ...
                .onAppear {
                    guard let _tempTask = Binding($tempTask) else { return }
                    priority = _tempTask.wrappedValue.priority
                }
            
                Button {
                    if !isUserEditingATask {
                        onSave(MyTask3(name: title, priority: priority))
                    } else {
                        if isTempTaskNameValid {
                            guard let _tempTask = Binding($tempTask) else { return }
                            _tempTask.wrappedValue.name = title
                ...
                .disabled(isUserEditingATask ? !isTempTaskNameValid : !isNewTaskTitleValid)
